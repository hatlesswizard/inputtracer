...
func (a *RustAnalyzer) ExtractClasses(root *sitter.Node, source []byte) ([]*types.ClassDef, error) {
	...
	for _, implNode := range implNodes {
		...
		if bodyNode != nil {
			methods := a.extractMethodsFromImpl(bodyNode, source)
...
func (a *RustAnalyzer) extractMethodsFromImpl(bodyNode *sitter.Node, source []byte) []*types.MethodDef {
