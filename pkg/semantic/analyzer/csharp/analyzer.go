...
func (a *CSharpAnalyzer) ExtractClasses(root *sitter.Node, source []byte) ([]*types.ClassDef, error) {
	...
	for _, classNode := range classNodes {
		...
		if bodyNode != nil {
			methods := a.extractMethodsFromClass(bodyNode, source)
...
func (a *CSharpAnalyzer) extractMethodsFromClass(bodyNode *sitter.Node, source []byte) []*types.MethodDef {
