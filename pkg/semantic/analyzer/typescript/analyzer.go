...
func (a *TypeScriptAnalyzer) registerFrameworkPatterns() {
	// TypeScript uses JavaScript patterns (Express, NestJS, etc.) with language override.
	base.RegisterFrameworkPatterns(a.BaseAnalyzer, jsPatterns.GetAllPatterns(), "typescript")
}
...
func (a *TypeScriptAnalyzer) AnalyzeMethodBody(method *types.MethodDef, source []byte, state *types.AnalysisState) (*analyzer.MethodFlowAnalysis, error) {
	return base.AnalyzeMethodBody(method, state, "", a.inputSources, tspatterns.BuildThisPropertyAssignPattern), nil
}
