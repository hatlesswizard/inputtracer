...
// traceBackwardInFile processes a single file for backward tracing
func (t *Tracer) traceBackwardInFile(filePath string, targetVar string) ([]types.BackwardPath, []types.SourceInfo) {
	paths := make([]types.BackwardPath, 0)
	sources := make([]types.SourceInfo, 0)
	...
	// Get AST on demand via parser service (LRU cached)
	root, content, err := t.getFileAST(filePath)
	if err != nil || root == nil {
		return paths, sources
	}
	...
	// Get all assignments in this file
	assignments, err := langAnalyzer.ExtractAssignments(root, content, "")
	if err != nil {
		return paths, sources
	}
	...
	for _, assign := range assignments {
		...
		if sourceInfo != nil {
			...
			if strings.HasPrefix(assign.Source, "$") {
				innerSources := t.traceBackwardRecursive(assign.Source, filePath, make(map[string]bool), 0)
				for _, innerSource := range innerSources {
					innerPath := types.BackwardPath{
						Source:     innerSource,
						Steps:      make([]types.BackwardStep, 0),
...
// traceBackwardRecursive recursively traces backward through variable assignments
func (t *Tracer) traceBackwardRecursive(varExpr string, startFile string, visited map[string]bool, depth int) []types.SourceInfo {
	...
	// Lock for reading t.files to prevent concurrent map access
	t.mu.RLock()
	// Copy file paths to iterate without holding lock during analysis
	filesCopy := make(map[string]*FileInfo, len(t.files))
	...
	// Search all files
	for filePath, fileInfo := range filesCopy {
		if fileInfo == nil {
			continue
		}
		...
		for _, assign := range assignments {
			...
			if sourceInfo := t.identifySource(assign.Source, filePath, assign.Line); sourceInfo != nil {
				...
				// Recurse into the source variable
				innerSources := t.traceBackwardRecursive(assign.Source, filePath, visited, depth+1)
...
// getFileAST returns the AST and content for a file by parsing fresh each time
// This is thread-safe because each call creates its own AST tree (tree-sitter nodes
// have internal caching that isn't thread-safe when shared between goroutines)
func (t *Tracer) getFileAST(filePath string) (*sitter.Node, []byte, error) {
	// Detect language from file path
	lang := t.parserService.DetectLanguage(filePath)
	if lang == "" {
	...
	// Parse fresh (no cache) - creates a new AST tree for this goroutine
	// We use parserService.Parse() which uses parser pools but doesn't cache the AST
	root, err := t.parserService.Parse(content, lang)
	if err != nil {
		return nil, nil, err
	}
	if root == nil {
...
// NOTE: getFileASTWithContext was removed to fix memory explosion
// Instead, use ctx.getAssignmentsDirectly() which parses → extracts → immediately discards AST
// This keeps memory usage low: only assignments (tiny) are cached, not ASTs (huge)

// parseFile parses a single file (creates its own parser - use parseFileWithParser for reuse)
func (t *Tracer) parseFile(path string) {
	lang := detectLanguage(path)
	if lang == "" {
		return
	}
