...
func (t *VariableTracer) traceDefinition(varName string, def VariableDefinition) VariableTraceResult {
	result := VariableTraceResult{
		...
		Assignments:  make([]VarAssignment, 0),
...
func extractJustVariable(varExpr string) string {
	...
	before, _, found := strings.Cut(varExpr, "[")
	if !found {
		return varExpr
	}
	return before
...
func (t *VariableTracer) findAssignmentsInScope(root *sitter.Node, content []byte, lines []string, varName, funcName, className string) []VarAssignment {
...
func (t *VariableTracer) walkForAssignments(node *sitter.Node, content []byte, lines []string, varName string, assignments *[]VarAssignment) {
	if node == nil {
		return
	}
...
func (t *VariableTracer) buildFlowPath(varName string, assignments []VarAssignment) []string {
	var path []string
