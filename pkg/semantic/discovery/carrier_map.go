...
func (m *CarrierMap) FindCarriersBySourceType(sourceType string) []InputCarrier {
	var carriers []InputCarrier
	for _, c := range m.Carriers {
		if slices.Contains(c.SourceTypes, sourceType) {
			carriers = append(carriers, c)
		}
	}
	return carriers
