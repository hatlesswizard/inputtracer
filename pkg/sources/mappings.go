...
// convertToTypesSourceType converts sources.SourceType map to common.SourceType map
func convertToTypesSourceType(m map[string]SourceType) map[string]common.SourceType {
	if m == nil {
		return make(map[string]common.SourceType)
	}
	result := make(map[string]common.SourceType, len(m))
	for k, v := range m {
		result[k] = common.SourceType(v)
	}
	return result
}
