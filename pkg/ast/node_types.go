// Package ast - node_types.go provides centralized AST node type patterns.
// All AST node type definitions should be referenced from here.
// This is language-agnostic structural knowledge about how ASTs are shaped —
// it belongs in pkg/ast, not in pkg/sources (input-detection logic).
package ast
...
// Used by propagation.go findContainingFunction() and getCurrentScope().
...
func IsFunctionNode(nodeType string) bool {
	return slices.Contains(UniversalASTNodeTypes.FunctionTypes, nodeType)
}
...
func IsFunctionNodeForLanguage(nodeType, language string) bool {
	...
	if langTypes, ok := LanguageASTNodeTypes[language]; ok {
		if slices.Contains(langTypes.FunctionTypes, nodeType) {
			return true
		}
	}
...
func IsScopeNode(nodeType string) bool {
	return slices.Contains(UniversalASTNodeTypes.ScopeTypes, nodeType)
}
...
func IsScopeNodeForLanguage(nodeType, language string) bool {
	// Check language-specific first
	if langTypes, ok := LanguageASTNodeTypes[language]; ok {
		if slices.Contains(langTypes.ScopeTypes, nodeType) {
			return true
		}
	}
	// Fall back to universal
...
func IsAssignmentNode(nodeType string) bool {
	return slices.Contains(UniversalASTNodeTypes.AssignmentTypes, nodeType)
}

// IsCallNode checks if a node type is a function/method call
func IsCallNode(nodeType string) bool {
	return slices.Contains(UniversalASTNodeTypes.CallTypes, nodeType)
}

// IsIdentifierNode checks if a node type is an identifier
func IsIdentifierNode(nodeType string) bool {
	return slices.Contains(UniversalASTNodeTypes.IdentifierTypes, nodeType)
}
...
func IsIdentifierNodeForLanguage(nodeType, language string) bool {
	if langTypes, ok := LanguageASTNodeTypes[language]; ok {
		if slices.Contains(langTypes.IdentifierTypes, nodeType) {
			return true
		}
	}
...
func IsAssignmentNodeForLanguage(nodeType, language string) bool {
	if langTypes, ok := LanguageASTNodeTypes[language]; ok {
		if slices.Contains(langTypes.AssignmentTypes, nodeType) {
			return true
		}
	}
...
func IsCallNodeForLanguage(nodeType, language string) bool {
	if langTypes, ok := LanguageASTNodeTypes[language]; ok {
		if slices.Contains(langTypes.CallTypes, nodeType) {
			return true
		}
	}
